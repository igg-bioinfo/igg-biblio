...
if user.login():
    ...
    if demo.update_count_filter == 0:
        st.error('Senza una anagrafica per l\'anno corrente non è possibile effettuare alcuna ulteriore operazione.')
    else:
        def set_pubs_row(obj, filter = ''):
            if obj.get_update_details():
                st.write("Ultimo aggiornamento: **" + str(obj.update_date) + " ("+ str(obj.update_days) + " giorni fa)**")
                st.write("Pubblicazioni" + filter + ": **" + str(obj.update_count_pubs) + "**")
                st.write("Autori per tutte le pubblicazioni" + filter + ": **" + str(obj.update_count_authors) + "**")
        ...
        if scopus_albo.get_metrics_update_details():
            ...
            st.error('Senza una anagrafica per l\'anno selezionato non è possibile effettuare alcuna ulteriore operazione.')
        ...
        if scopus_pucs.get_pucs_update_details():
            ...
            st.success("Non ci sono PUC mancanti per le pubblicazioni di quest'anno.")
        ...
        set_pubs_row(pubmed, ' (senza ahead of print)')
